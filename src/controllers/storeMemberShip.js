...
export const removeStoreMemberShip = async (req, res) => {
    try {
        const storeMemberShip = await StoreMemberShip.findOneAndDelete({ _id: req.params.id }).exec();
        res.json(storeMemberShip);
    } catch (error) {
        res.status(400).json({
            message: error.message
...
export const updateStoreMemberShip = async (req, res) => {
    ...
    try {
        const storeMemberShip = await StoreMemberShip.findOneAndUpdate({ _id: req.params.id }, req.body, option).exec();
        res.json(storeMemberShip);
    } catch (error) {
        console.log(error)
        res.status(400).json({
...
export const readStoreMemberShip = async (req, res) => {
    try {
        const storeMemberShip = await StoreMemberShip.findOne({ _id: req.params.id }).exec();
        res.json(storeMemberShip);
    } catch (error) {
        res.status(400).json({
            message: error.message
